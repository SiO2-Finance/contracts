...
import {ISiO2Oracle} from './interfaces/ISiO2Oracle.sol';
...
contract UiPoolDataProviderV2 is IUiPoolDataProviderV2 {
  ...
  {
    ISiO2Oracle oracle = ISiO2Oracle(provider.getPriceOracle());
    ILendingPool lendingPool = ILendingPool(provider.getLendingPool());
    ...
    returns (UserReserveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      userReservesData[i].scaledSTokenBalance = ISToken(baseData.STokenAddress).scaledBalanceOf(
        user
      );
